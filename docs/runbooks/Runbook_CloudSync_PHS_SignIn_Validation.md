...
6. status/errorCode = 0 の成功イベントを確認する。
...
| status/errorCode = 0 | Sign-in logs上の成功イベント |
| MFA/CAで停止 | PHS失敗とは限らない。Sign-in logsで切り分ける。 |
| パスワード不正 | PHS未反映、入力誤り、AD側設定不備を確認する。 |

## 50140等の割り込みイベント

Keep Me Signed Inなどの割り込みイベントが記録される場合があります。この場合でも、後続でstatus/errorCode = 0の成功イベントが確認できれば、PHS失敗とは扱いません。
