...
| 領域 | このリポジトリで扱うこと | 実務で別途定義すること |
|---|---|---|
| Microsoft 365 / Entra ID | ID、Conditional Access、MFA、同期ユーザー確認の設計観点を整理 | テナント運用権限、Break-glass、監査ログ保持、変更承認 |
| AD DS / Cloud Sync | ラボADからEntra IDへの選定ユーザー同期とPHS検証 | OU設計、gMSA運用、Agent冗長化、Tier 0保護、復旧計画 |
| AVD / Intune | Microsoft Entra joined Session Host、Intune準拠、Start VM on Connect、停止手順 | FSLogix、スケール設計、監視、ユーザー通知、メンテナンス窓 |
| 証跡 / 公開情報 | Markdown要約とマスキング方針を公開 | スクリーンショット原本、CSV/JSON原本、実ID、UPN、IP、トークン類の非公開保管 |

この構成は、顧客環境の設定値や運用ログを再掲するものではありません。公開版では、再現性とレビュー性を残しつつ、実環境を特定できる情報を持ち込まない方針にしています。
